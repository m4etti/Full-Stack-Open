...
describe('PUT /api/blogs/:id', () => {
    ...
    test('adding like to someone else\'s blog returns 200', async () => {
        const blogsAtStart = await helper.blogsInDb()
        const blogToUpdate = blogsAtStart[0]

        // find a user that is not the owner of the blog
        const user = usersInDb.find(user => user.id !== blogToUpdate.user.toString())

        const likes = { likes: blogToUpdate.likes + 1 }
    ...
    test('user that has been removed cannot add likes', async () => {
        const blogsAtStart = await helper.blogsInDb()
        const blogToLike = blogsAtStart[0]
        const user = usersInDb[2]

        // Remove the user from the database
        await User.findByIdAndRemove(user.id)

        const likes = { likes: blogToLike.likes + 1 }
        ...
        expect(updatedBlog.likes).toEqual(blogToLike.likes)
    })
})
